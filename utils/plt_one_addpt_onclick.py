...
from utils.lab_utils_common_classification import np, plt, dlblue, dlorange, sigmoid, dldarkred, gradient_descent
...
#   @output.capture()  # debug
    ...
    def resize_sq(self, bcid):
        """Resize the checkbox square (version-compatible)."""

        # 1) 取得 checkbox 的方框（不同 Matplotlib 版本命名不同）
        rects = getattr(bcid, "rectangles", None) or \
                getattr(bcid, "boxes", None) or \
                getattr(bcid, "_rectangles", None)

        if rects is None:
            # 最後手段：從軸上的 patches 抓第一個矩形
            rects = [p for p in getattr(bcid.ax, "patches", []) if hasattr(p, "get_height")]
        ...
        try:
            ...
            # 有些後端或型別可能沒有 get_bbox
            return

        # 3) 調整打勾線段的 y 範圍（不同版本 lines 結構可能不同）
        lines = getattr(bcid, "lines", None)
        if lines and len(lines) > 0:
            try:
                # 常見結構：lines[0] 是 (Line2D, Line2D)
                l0, l1 = lines[0][0], lines[0][1]
                l0.set_ydata([ymax, ymin])
                l1.set_ydata([ymin, ymax])
            except Exception:
                # 有些版本結構不同，直接略過
                pass
